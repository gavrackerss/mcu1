// TeslaGatewayV497R1AP318StartupWakeMin300SerializedReconciliationBenchBuilderV617R2.java
//
...
//
// V617R2 state machine at scratch 0x40014E2C
// ------------------------------------------
...
// bit     15 : DONE (terminal state = 0x8000)
//
...
// @menupath Tools.Tesla.Build V497R1 AP318 Serialized Reconciliation V617R2
...
public class TeslaGatewayV497R1AP318StartupWakeMin300SerializedReconciliationBenchBuilderV617R2
        extends GhidraScript {

    private static final String PREFIX =
        "TeslaGatewayV497R1AP318StartupWakeMin300SerializedReconciliationBenchBuilderV617R2";
    ...
    private static final int DONE_STATE = 0x8000;
    ...
    protected void run() throws Exception {
        ...
        if(!STOCK_SHA256.equalsIgnoreCase(stockSha))
            throw new IllegalStateException(
                "V617R2 requires the exact stock analysed Ghidra project. SHA256="+stockSha);

        Contract c=validateAndBuildStockContract();

        File input=askFile("Select exact V497R1 2 MiB BIN","Open");
        if(input==null)return;
        File outDir=askDirectory("Select NEW EMPTY V617R2 output folder","Select");
        ...
        if(compensation==null)
            throw new IllegalStateException("V617R2 CRC-neutral compensation solver failed.");
        putBytes(patched,COMPENSATION_ADDRESS,compensation);

        validateOutput(baseline,patched,c,compensation);

        String stem=
            "Tesla_MCU1_V497R1_AP318SerializedReconciliation_V617R2_CRCNeutral";
    ...
    private Contract validateAndBuildStockContract() throws Exception {
        ...
        if(wakeCall==null || wakeCall.getLength()!=4 ||
           ...
           wakeCall.getFlowType().isComputed())
            throw new IllegalStateException(
                "V617R2 expected direct 4-byte A47EA event-send call at "+hex(WAKE_EDGE_SEND_CALL)+".");
        Address[] wakeFlows=wakeCall.getFlows();
        if(wakeFlows==null || wakeFlows.length!=1 ||
           (wakeFlows[0].getOffset()&0xffffffffL)!=EVENT_SEND)
            throw new IllegalStateException(
                "V617R2 A47EA wake-edge call does not target F7942.");
        ...
        if(critEnter==null || (critEnter.getEntryPoint().getOffset()&0xffffffffL)!=CRITICAL_ENTER)
            throw new IllegalStateException("V617R2 critical-enter 0xF7380 is not defined as expected.");
        if(critExit==null || (critExit.getEntryPoint().getOffset()&0xffffffffL)!=CRITICAL_EXIT)
            throw new IllegalStateException("V617R2 critical-exit 0xF73A8 is not defined as expected.");
        ...
        if(c.apWrapper.length>AP_WRAPPER_RESERVED)
            throw new IllegalStateException(
                "V617R2 AP wrapper exceeds reserved cave: "+c.apWrapper.length);
        if(c.oneHzWrapper.length>ONEHZ_WRAPPER_RESERVED)
            throw new IllegalStateException(
                "V617R2 oneHz wrapper exceeds reserved cave: "+c.oneHzWrapper.length);
        if(c.wakeWrapper.length>WAKE_WRAPPER_RESERVED)
            throw new IllegalStateException(
                "V617R2 wake wrapper exceeds reserved cave: "+c.wakeWrapper.length);
        if(c.stateHelper.length>STATE_HELPER_RESERVED)
            throw new IllegalStateException(
                "V617R2 state helper exceeds reserved cave: "+c.stateHelper.length);
        if(c.cmd32Wrapper.length>CMD32_WRAPPER_RESERVED)
            throw new IllegalStateException(
                "V617R2 command32 wrapper exceeds reserved cave: "+c.cmd32Wrapper.length);
    ...
    private byte[] buildApWrapper(Contract c) throws Exception {
        ...
        return assembleBlock(AP_WRAPPER,
            ...
            ins("se_btsti r0,"+bitOperand(ACTION_0800),2),
            br("e_bne cr0,","INIT_STORE",4),
            ins("se_btsti r0,"+bitOperand(ACTION_100000),2),
            br("e_bne cr0,","INIT_STORE",4),
            ins("se_btsti r0,"+bitOperand(ACTION_400000),2),
            br("e_bne cr0,","INIT_STORE",4),
            ins("e_or2i r12,"+imm16(CLEAR_BASELINE),4),
            mark("INIT_STORE"),
            ins("e_stw r12,0(r11)",4),

            mark("STATE_READY"),
            // r8=1 means session active/AP0; DONE exact bit means AP2.
            ins("e_li r8,0x1",4),
            ins("e_xori r7,r12,"+imm16(DONE_STATE),4),
            ins("e_cmp16i. r7,0x0",4),
    ...
    private byte[] buildWakeWrapper(Contract c) throws Exception {
        ...
        // Preserve original F7942 arguments and caller state.
        p=emit(out,p,"e_stwu r1,-64(r1)",4);
        p=emit(out,p,"e_stw r0,4(r1)",4);
        p=emit(out,p,"e_stw r3,8(r1)",4);
        p=emit(out,p,"e_stw r4,12(r1)",4);
        ...
        // New generation/session: overwrite every old state flag.
        p=emit(out,p,"e_li r12,"+imm16(WINDOW_TICKS),4);
        // Seed CLEAR_BASELINE only if the exact ACTION surface is clear now.
        p=emit(out,p,"e_lis r11,"+hi16(ACTION_WORD),4);
        p=emit(out,p,"e_add16i r11,r11,"+lo16(ACTION_WORD),4);
        p=emit(out,p,"e_lwz r0,0(r11)",4);
        long storeState=WAKE_WRAPPER+0x58L;
        p=emit(out,p,"se_btsti r0,"+bitOperand(ACTION_0800),2);
        p=emit(out,p,"e_bne cr0,"+hex(storeState),4);
        p=emit(out,p,"se_btsti r0,"+bitOperand(ACTION_100000),2);
        p=emit(out,p,"e_bne cr0,"+hex(storeState),4);
        p=emit(out,p,"se_btsti r0,"+bitOperand(ACTION_400000),2);
        p=emit(out,p,"e_bne cr0,"+hex(storeState),4);
        p=emit(out,p,"e_or2i r12,"+imm16(CLEAR_BASELINE),4);
        if((p.getOffset()&0xffffffffL)!=storeState)
            throw new IllegalStateException("V617R2 wake store-state label drifted: "+p+" expected "+hex(storeState));
        ...
        // Clear ONLY CAN318 AP bits before the original observed event send.
        ...
        // Reconstruct the exact original F7942 call arguments.
    ...
    private byte[] buildStateHelper() throws Exception {
        // Called before the remainder of stock oneHz, therefore before 0x7150E
        // -> A31E4 can clear ACTION. The command wrapper also post-samples 0x88340.
        return assembleBlock(STATE_HELPER,
            ...
            // Current exact ACTION surface in r0.
            ins("e_lis r10,"+hi16(ACTION_WORD),4),
            ins("e_add16i r10,r10,"+lo16(ACTION_WORD),4),
            ins("e_lwz r0,0(r10)",4),

            // Establish a post-rearm clear baseline before any set can qualify.
            ins("e_andi. r7,r12,"+imm16(CLEAR_BASELINE),4),
            ...
            mark("CHECK_EPISODE"),
            ins("e_andi. r7,r12,"+imm16(CMD32_EPISODE),4),
            br("e_beq cr0,","COUNTDOWN",4),
            // Accumulate exactly whichever conditional ACTION subset appears.
            ins("se_btsti r0,"+bitOperand(ACTION_0800),2),
            br("e_beq cr0,","SEEN_100000_TEST",4),
            ins("e_or2i r12,"+imm16(SEEN_0800),4),
            mark("SEEN_100000_TEST"),
            ins("se_btsti r0,"+bitOperand(ACTION_100000),2),
            br("e_beq cr0,","SEEN_400000_TEST",4),
            ins("e_or2i r12,"+imm16(SEEN_100000),4),
            mark("SEEN_400000_TEST"),
            ins("se_btsti r0,"+bitOperand(ACTION_400000),2),
            br("e_beq cr0,","COUNTDOWN",4),
            ins("e_or2i r12,"+imm16(SEEN_400000),4),
            ...
            mark("ELAPSED"),
            ins("e_or2i r12,"+imm16(MINIMUM_ELAPSED),4),
            // Require baseline + marked command episode + nonzero observed subset.
            ins("e_andi. r7,r12,"+imm16(CLEAR_BASELINE),4),
            br("e_beq cr0,","COMMIT",4),
            ins("e_andi. r7,r12,"+imm16(CMD32_EPISODE),4),
            br("e_beq cr0,","COMMIT",4),
            ins("e_andi. r7,r12,"+imm16(SEEN_MASK),4),
            br("e_beq cr0,","COMMIT",4),
            // Conservative completion: all three exact ACTION bits must now be clear.
            ins("se_btsti r0,"+bitOperand(ACTION_0800),2),
            br("e_bne cr0,","COMMIT",4),
            ins("se_btsti r0,"+bitOperand(ACTION_100000),2),
            br("e_bne cr0,","COMMIT",4),
            ...
            // DONE and direct AP2 restore occur inside the same critical section.
    ...
    private byte[] buildCmd32Wrapper() throws Exception {
        // Exact V619R1 episode marker wrapper. Stock 0x88340 is called once.
        // Pre-call: establish baseline if clear and mark the first eligible episode.
        // Post-call: immediately accumulate the conditional subset set by 0x88340.
        return assembleBlock(CMD32_WRAPPER,
            ...
            // If baseline is not yet latched, this exact command boundary may
            // establish it only when all ACTION bits are clear before 0x88340.
            ins("e_andi. r7,r12,"+imm16(CLEAR_BASELINE),4),
            ...
            mark("MARK_EPISODE"),
            // Do not reset a previously marked in-flight episode.
            ins("e_andi. r7,r12,"+imm16(CMD32_EPISODE),4),
            br("e_bne cr0,","PRE_STORE",4),
            ins("e_or2i r12,"+imm16(CMD32_EPISODE),4),
            mark("PRE_STORE"),
            ins("e_stw r12,0(r11)",4),
            ...
            // Immediate post-call subset capture. oneHz pre-body helper provides
            // the complementary capture path if scheduling occurred during 0x88340.
            ins("e_bl "+hex(CRITICAL_ENTER),4),
            ins("e_lis r11,"+hi16(SCRATCH_WORD),4),
            ins("e_add16i r11,r11,"+lo16(SCRATCH_WORD),4),
            ins("e_lwz r12,0(r11)",4),
    ...
    private void validateV497R1Baseline(byte[] image,Contract c) throws Exception {
        ...
        if(!V497R1_INPUT_SHA256.equalsIgnoreCase(sha))
            throw new IllegalStateException(
                "V617R2 requires exact V497R1 input BIN. SHA256="+sha);
    ...
    private void validateOutput(byte[] baseline,byte[] patched,Contract c,
        ...
        if(stored!=STOCK_APP_CRC || calculated!=STOCK_APP_CRC)
            throw new IllegalStateException(String.format(Locale.ROOT,
                "V617R2 CRC mismatch. stored=0x%08X calculated=0x%08X",
                stored,calculated));
        ...
        for(int i=0;i<IMAGE_SIZE;i++){
            ...
            if(!allowed)
                throw new IllegalStateException(
                    "Unexpected V617R2 changed byte at "+hex(a)+".");
        }
    }
    ...
    private void writeReport(File report,File input,String stockSha,
        ...
        try(BufferedWriter w=new BufferedWriter(new OutputStreamWriter(
            ...
            w.write("Tesla Gateway V497R1 AP318 Serialized Reconciliation V617R2\r\n");
            w.write("=============================================================\r\n\r\n");
            ...
            w.write("bits0..8 countdown; bit9 CLEAR_BASELINE; bit10 CMD32_EPISODE; bits11..13 observed ACTION 0x800/0x100000/0x400000; bit14 MINIMUM_ELAPSED; bit15 DONE.\r\n");
    ...
    private String bitOperand(long oneBitMask){
    ...
    private void requireEmptyDirectory(File d) {
        ...
        if(f!=null&&f.length!=0)
            throw new IllegalStateException(
                "V617R2 BIN/S19 builder requires a NEW EMPTY output directory.");
    }
